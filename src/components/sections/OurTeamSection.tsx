...
export const TeamCard = ({ name, jobTitle, imageUrl }: TeamCardProps) => {
    return (
        <div className="flex border-gray-300 border-solid border max-w-lg w-full">
            <div className="w-52 h-40 bg-center bg-no-repeat bg-cover " style={{ backgroundImage: `url(${imageUrl})`, width: '208px' }}></div>
...
const OurTeamSection = ({ all = false, }: OurTeamSectionProps) => {
        ...
        <section className='py-20 px-5'>
            <div className="container mx-auto space-y-10">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-10 justify-items-center">
                    {
                        tData.map(data => {
                            return <TeamCard key={nanoid()} {...data} />
                        })
                    }
                </div>
            </div>
